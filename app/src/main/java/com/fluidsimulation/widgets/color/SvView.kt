package com.fluidsimulation.widgets.color
...
@JvmOverloads constructor(
    ...
    fun setColor(@ColorInt color: Int) {
        this.color = color
        colorToHsv(color, hsvCache)
        updateHue(hsvCache[0])
        updateSv(hsvCache[1], hsvCache[2])
    }

    fun setHue(h: Float) {
        color = hsvToColor(h, saturation, value)
        updateHue(h)
    }
            ...
            repeat(TONE_SIZE) { y ->
                repeat(TONE_SIZE) { x ->
                    pixels[x + y * TONE_SIZE] =
                        svToMask(x / TONE_MAX, (TONE_MAX - y) / TONE_MAX)
                }
            }
